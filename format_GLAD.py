...
for i in mod44w_tilelist:
    ...
    modext = arcpy.Describe(i).extent
    modtilebbox = arcpy.Multipoint(arcpy.Array([modext.lowerLeft,modext.lowerRight, modext.upperLeft, modext.upperRight]),
                                   mod44w_sr)

    #Project extent to GLAD's projection (here wgs84)
    extpoly = arcpy.Project_management(in_dataset=modtilebbox,
                                       out_dataset=os.path.join(mod44w_resgdb, 'extbbox'),
                                       out_coor_system=rawtilelist[0])
    #Add extebt to dictionary as value with MODIS tile id as the key
    mod44w_wgsextdict[i] = arcpy.Describe(extpoly).extent
    del extpoly

arcpy.Delete_management(os.path.join(mod44w_resgdb, 'extpoly'))
...
for gladtile in rawtilelist:
    ...
    gladtileid = re.sub('(^.*class99_1[89]_)|([.]tif)', '', gladtile) #Unique identifier for glad tile based on coordinates
    ...
    if not Raster(mod44w_gladmatch).hasRAT and arcpy.Describe(mod44w_gladmatch).bandCount == 1:  # Build attribute table if doesn't exist
        try:
            arcpy.BuildRasterAttributeTable_management(mod44w_gladmatch)  # Does not work
        except Exception:
            e = sys.exc_info()[1]
            print(e.args[0])
            arcpy.DeleteRasterAttributeTable_management(mod44w_gladmatch)

    mod44wvals = {row[0] for row in arcpy.da.SearchCursor(mod44w_gladmatch, 'Value')}

    if 4 in mod44wvals: #If there are sea pixels in MODIS, then create a seamask
        print('Creating seamask...')
        #Run a 3x3 majority filter on MODIS to get rid of isolated sea pixels
        print('Majority filter of MODIS QA layer...')
        ...
        print('Project and resample MODIS to match GLAD tile...')
